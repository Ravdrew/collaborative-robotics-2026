...
class StateMachineNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("fruit_target_local_topic", "/fruit_target_local")
        self.declare_parameter("pick_target_local_pose_topic", "/pick_target_local")
        self.declare_parameter("place_target_local_topic", "/place_target_local")
        self.declare_parameter("place_target_local_point_topic", "/place_target_local_point")
        self.declare_parameter("place_target_local_pose_topic", "/place_target_local_pose")

        # Frame and filter parameters.
        self.declare_parameter("map_frame", "map")
        self.declare_parameter("default_camera_frame", "camera_color_optical_frame")
        self.declare_parameter("assume_pose_topics_in_frame", "camera_color_optical_frame")
        self.declare_parameter("target_filter", "banana")
        self.declare_parameter("require_pick_target_match", True)
        ...
        # Max number of pick reattempts before giving up and staying in PICKING indefinitely.
        # Set to 0 to disable (wait forever).  Each failed /successful_pick=false triggers a
        # new /fsm_pick_request so the arm controller gets another chance.
        ...
        self.current_pick_target: Optional[str] = None
        self.current_place_target: Optional[str] = None
        ...
        self.assume_pose_topics_in_frame = str(
            self.get_parameter("assume_pose_topics_in_frame").value
        )
        self.target_filter = str(self.get_parameter("target_filter").value).strip().lower()
        self.require_pick_target_match = bool(
            self.get_parameter("require_pick_target_match").value
        )
        ...
        self.event_counts = {
            ...
            "fruit_target_local": 0,
            "pick_target_local_pose": 0,
            "place_target_local": 0,
            "place_target_local_point": 0,
            "place_target_local_pose": 0,
            "cmd_nav_sent": 0,
            "nav_success": 0,
            "successful_pick": 0,
        ...
        self.create_subscription(
            PointStamped,
            str(self.get_parameter("fruit_target_local_topic").value),
            self._on_fruit_target_local,
            10,
        )
        self.create_subscription(
            Pose,
            str(self.get_parameter("pick_target_local_pose_topic").value),
            self._on_pick_target_local_pose,
            10,
        )
        self.create_subscription(
            PoseStamped,
        ...
        self.create_subscription(
            PointStamped,
            str(self.get_parameter("place_target_local_point_topic").value),
            self._on_place_target_local_point,
            10,
        )
        self.create_subscription(
            Pose,
            str(self.get_parameter("place_target_local_pose_topic").value),
            self._on_place_target_local_pose,
            10,
        )
        ...
        self.get_logger().info(
            f"Pick filter target='{self.target_filter}' require_match={self.require_pick_target_match}"
        )
    ...
    def _on_pick_target(self, msg: String):
        ...
        # pick target text from audio
        pick_text = msg.data.strip().lower()
        self.current_pick_target = pick_text if pick_text else None
        # keep old gate behavior
        if msg.data.strip():
    ...
    def _on_place_target(self, msg: String):
        ...
        # place target text from audio
        place_text = msg.data.strip().lower()
        self.current_place_target = place_text if place_text else None
        # keep old gate behavior
        if msg.data.strip():
    ...
    def _on_pick_target_global(self, msg: PointStamped):
        ...
        if self.state != SMState.PICK_NAVIGATION:
            ...
            return
        self._handle_pick_point_stamped(msg, "/pick_target_global")

    # Convert fruit target point into Pose2D goal.
    def _on_fruit_target_local(self, msg: PointStamped):
        self.event_counts["fruit_target_local"] += 1
        if self.state != SMState.PICK_NAVIGATION:
            self.get_logger().warn(
                f"Ignoring /fruit_target_local in state={self.state.value}"
            )
            return
        self._handle_pick_point_stamped(msg, "/fruit_target_local")

    # Convert pick pose into Pose2D goal.
    def _on_pick_target_local_pose(self, msg: Pose):
        self.event_counts["pick_target_local_pose"] += 1
        if self.state != SMState.PICK_NAVIGATION:
            self.get_logger().warn(
                f"Ignoring /pick_target_local in state={self.state.value}"
            )
            return
        # gate pick goals by target text
        if not self._pick_target_allowed():
            self.event_counts["pick_filter_drop"] += 1
            return
        # pose has no header so assume camera frame
        point_msg = self._point_stamped_from_pose(
            msg, self.assume_pose_topics_in_frame
        )
        goal = self._pose2d_from_point_stamped(point_msg, "/pick_target_local")
        if goal is None:
            return
        self._publish_cmd_nav(goal, phase="pick", reason="converted /pick_target_local")
    ...
    def _on_place_target_local(self, msg: PoseStamped):
        ...
        if self.state != SMState.PLACE_NAVIGATION:
            ...
            return
        # convert posestamped to point first
        point_msg = self._point_stamped_from_pose_stamped(msg)
        goal = self._pose2d_from_point_stamped(point_msg, "/place_target_local")
        if goal is None:
            return
        self._publish_cmd_nav(goal, phase="place", reason="converted /place_target_local")

    # Convert place point into Pose2D goal.
    def _on_place_target_local_point(self, msg: PointStamped):
        self.event_counts["place_target_local_point"] += 1
        if self.state != SMState.PLACE_NAVIGATION:
            self.get_logger().warn(
                f"Ignoring /place_target_local_point in state={self.state.value}"
            )
            return
        goal = self._pose2d_from_point_stamped(msg, "/place_target_local_point")
        if goal is None:
            return
        self._publish_cmd_nav(goal, phase="place", reason="converted /place_target_local_point")

    # Convert place pose into Pose2D goal.
    def _on_place_target_local_pose(self, msg: Pose):
        self.event_counts["place_target_local_pose"] += 1
        if self.state != SMState.PLACE_NAVIGATION:
            self.get_logger().warn(
                f"Ignoring /place_target_local_pose in state={self.state.value}"
            )
            return
        # pose has no header so assume camera frame
        point_msg = self._point_stamped_from_pose(
        ...
        goal = self._pose2d_from_point_stamped(point_msg, "/place_target_local_pose")
        if goal is None:
            return
        self._publish_cmd_nav(goal, phase="place", reason="converted /place_target_local_pose")

    # Transition nav states when navigator reports success.
    def _on_nav_success(self, msg: Bool):
        self.event_counts["nav_success"] += 1
        if not bool(msg.data):
            # keep waiting if nav not done
            self.get_logger().warn("Received /nav_success=false; staying in current state")
            return

        # Ignore late arrivals after the task is complete.
        if self.state == SMState.FINISHED:
            return

        # nav success needs active phase
    ...
    def _on_successful_pick(self, msg: Bool):
        ...
        if bool(msg.data):
            ...
            # Reattempt up to max_pick_attempts by re-sending /fsm_pick_request.
            # If max_pick_attempts==0 the FSM waits indefinitely (useful when the
            # arm controller manages its own retry loop and will eventually succeed).
            if self.max_pick_attempts > 0 and self.pick_attempt_count < self.max_pick_attempts:
                self.pick_attempt_count += 1
                self.get_logger().warn(
                    f"successful_pick=false; retrying pick "
                    f"(attempt {self.pick_attempt_count}/{self.max_pick_attempts})"
                )
                self._publish_pick_request_once()
            else:
                self.get_logger().warn(
                    f"successful_pick=false; max attempts reached "
                    f"({self.max_pick_attempts}), staying in PICKING and waiting"
                )
    ...
    # choose if pick target should drive nav
    def _pick_target_allowed(self) -> bool:
        if not self.require_pick_target_match:
            return True
        if not self.current_pick_target:
            self.get_logger().warn("Pick target filter enabled but no pick target text yet")
            return False
        if self.current_pick_target != self.target_filter:
            self.get_logger().info(
                f"Skip pick nav for target='{self.current_pick_target}' filter='{self.target_filter}'"
            )
            return False
        return True

    # route pick point topic to cmd_nav
    def _handle_pick_point_stamped(self, msg: PointStamped, source_topic: str):
        # gate pick goals by target text
        if not self._pick_target_allowed():
            self.event_counts["pick_filter_drop"] += 1
            return
        goal = self._pose2d_from_point_stamped(msg, source_topic)
        if goal is None:
            return
        self._publish_cmd_nav(goal, phase="pick", reason=f"converted {source_topic}")

    # convert pointstamped to pose2d in map
    def _pose2d_from_point_stamped(
        self, msg: PointStamped, source_topic: str
    ) -> Optional[Pose2D]:
        # convert point to map frame
        map_point = self._to_map_point(msg, source_topic)
        if map_point is None:
            return None
        # map xy to cmd_nav; theta=0.0 because the navigator is expected to
        # determine approach orientation autonomously.  If the place phase ever
        # needs a specific facing direction (e.g. toward the human), compute it
        # here or pass it through the goal topic before publishing.
        goal = Pose2D()
        goal.x = float(map_point.point.x)
        goal.y = float(map_point.point.y)
        goal.theta = 0.0
        return goal

    # build point from pose without header
    def _point_stamped_from_pose(self, msg: Pose, frame_id: str) -> PointStamped:
        point_msg = PointStamped()
        point_msg.header.stamp = self.get_clock().now().to_msg()
        point_msg.header.frame_id = frame_id
        point_msg.point.x = float(msg.position.x)
        point_msg.point.y = float(msg.position.y)
        point_msg.point.z = float(msg.position.z)
        return point_msg

    # build point from posestamped
    def _point_stamped_from_pose_stamped(self, msg: PoseStamped) -> PointStamped:
        frame_id = (msg.header.frame_id or "").strip() or self.default_camera_frame
        point_msg = PointStamped()
        # Use current time rather than the incoming stamp to avoid stale-TF
        # extrapolation errors when the publisher does not set a timestamp.
        point_msg.header.stamp = self.get_clock().now().to_msg()
        point_msg.header.frame_id = frame_id
        point_msg.point.x = float(msg.pose.position.x)
        point_msg.point.y = float(msg.pose.position.y)
        point_msg.point.z = float(msg.pose.position.z)
        return point_msg

    # transform point into map
    def _to_map_point(self, msg: PointStamped, source_topic: str) -> Optional[PointStamped]:
        frame_id = (msg.header.frame_id or "").strip() or self.default_camera_frame
        stamped = PointStamped()
        stamped.header.stamp = msg.header.stamp
        stamped.header.frame_id = frame_id
        stamped.point.x = float(msg.point.x)
        stamped.point.y = float(msg.point.y)
        stamped.point.z = float(msg.point.z)

        # skip tf when already in map
        if frame_id == self.map_frame:
            return stamped

        try:
            # lookup transform frame to map
            ...
            self.event_counts["transform_fail"] += 1
            self.get_logger().warn(
                f"TF failed for {source_topic}: {frame_id} -> {self.map_frame}: {exc}"
            )
            return None

        # Use tf2_geometry_msgs to apply the transform correctly.
    ...
    def _on_state_entry(self):
        if self.state in (SMState.PICK_NAVIGATION, SMState.PLACE_NAVIGATION):
            # Clear old nav phase and age on entry.
            # IMPORTANT: _on_state_entry is called from _transition before any
            # incoming message callbacks can fire, so resetting pending_nav_phase
            # here is safe.  If that ordering ever changes (e.g. executor threads),
            # a queued nav-goal message could set pending_nav_phase and then this
            # reset would wipe it, causing /nav_success to be silently ignored.
            self.pending_nav_phase = None
    ...
    def _heartbeat_log(self):
        ...
        self.get_logger().info(
            ...
            f"pick_target='{self.current_pick_target}' place_target='{self.current_place_target}' "
