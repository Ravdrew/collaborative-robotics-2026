...
class StateMachineNode(Node):
    ...
    def _start_nudge_timeout(self):
        ...
        self._cancel_nudge()
        self.nudge_timeout_timer = self.create_timer(10.0, self._begin_nudge, callback_group=None)
    ...
    def _begin_nudge(self):
        ...
        if self.nudge_timeout_timer is not None:
            self.nudge_timeout_timer.cancel()
            self.nudge_timeout_timer = None
        self.get_logger().info("No detection after 10s — starting nudge (turn left/right)")
        self.nudge_step = 0
